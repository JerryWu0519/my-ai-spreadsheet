...
import type { Provenance, SourceFileType } from '@/app/provenance/provenanceTypes';
import type {
  BorderStyle,
  CellAlign,
  CellVerticalAlign,
...
export const aiToolsActions: AIToolActionsRecord = {
  ...
  [AITool.AddDataTable]: async (args) => {
    try {
      ...
      // Deep-Traceability: extract provenance if present
      const provArgs = args as Record<string, unknown>;
      const provenanceMeta = provArgs._provenance as
        | {
            file_id?: string;
            file_name?: string;
            file_type?: string;
      ...
      if (table_data.length > 0 && table_data[0].length > 0) {
        ...
        // Deep-Traceability: ALWAYS store provenance for every cell in the data table
        {
          const sourceUrls = (provArgs._sourceUrls as Array<{ title: string; uri: string }>) || [];
          const responseText = (provArgs._responseText as string) || '';
          const primarySource = sourceUrls[0];

          console.log('[Deep-Traceability:AddDataTable] provArgs keys:', Object.keys(provArgs));
          console.log('[Deep-Traceability:AddDataTable] _sourceUrls:', sourceUrls);
          console.log('[Deep-Traceability:AddDataTable] provenanceMeta:', provenanceMeta);

          // Priority: 1) Gemini _provenance metadata, 2) grounding URLs, 3) fallback
          const baseProv: Omit<Provenance, 'location'> = {
            fileId: provenanceMeta?.source_url || primarySource?.uri || `ai-data-table-${Date.now()}`,
            fileName: provenanceMeta?.file_name || primarySource?.title || table_name || 'Data Source',
            fileType:
              provenanceMeta?.source_url || primarySource?.uri
                ? 'url'
                : (provenanceMeta?.file_type as SourceFileType) || 'text',
            extractedAt: new Date().toISOString(),
            extractedBy: provenanceMeta?.file_name || primarySource?.title || 'AI Analyst',
            sourceSnippet:
              provenanceMeta?.source_snippet || provenanceMeta?.text_anchor || responseText.slice(0, 300) || undefined,
            sourceUrl: provenanceMeta?.source_url || primarySource?.uri,
          };
          // Store all source URLs and grounding supports for the viewer
          if (sourceUrls.length > 0) {
            (baseProv as any)._allSourceUrls = sourceUrls;
          }
          const gSupports = (provArgs._groundingSupports as any[]) || [];
          if (gSupports.length > 0) {
            (baseProv as any)._groundingSupports = gSupports;
          }
          provenanceStore.setRange(sheetId, x, y, table_data, baseProv, (row, col) => ({
            rowIndex: row,
            columnRef: table_data[0]?.[col] ?? col,
            textAnchor: provenanceMeta?.text_anchor,
            pageIndex: provenanceMeta?.page_index,
            cellValue: table_data[row]?.[col] ?? undefined,
            cellLabel: table_data[row]?.[0] ?? undefined,
          }));
        }
  ...
  [AITool.SetCellValues]: async (args) => {
    try {
      ...
      if (cell_values.length > 0 && cell_values[0].length > 0) {
        ...
        // Pre-process cell_values: detect empty cells and formula cells.
        // Empty cells should not overwrite existing content (e.g., formulas
        // already set via SetFormulaCellValue). Formula cells (starting with '=')
        // should be routed through setFormula for proper evaluation.
        const formulaCells: Array<{ selection: string; codeString: string }> = [];
        const hasEmptyOrFormulaCells = cell_values.some((row) =>
          row.some((v) => {
        ...
        if (hasEmptyOrFormulaCells) {
          // Extract formula cells and build a filtered 2D array
          // where empty cells are replaced with a no-op sentinel
          // Strategy: write non-empty, non-formula cells via setCellValue individually,
          // and formula cells via setFormulas
          for (let row = 0; row < cell_values.length; row++) {
            for (let col = 0; col < cell_values[row].length; col++) {
              ...
              if (val === '') {
                // Skip empty cells — don't overwrite existing content
                continue;
              } else if (val.startsWith('=')) {
                // Formula cell — route through setFormulas for proper evaluation
                formulaCells.push({
                  selection: xyToA1(x + col, y + row),
                  codeString: val.slice(1),
                });
              } else {
                // Regular value — write individually
                quadraticCore.setCellValue(sheetId, x + col, y + row, val, true);
              }
            }
          }
          // Set all formula cells in a batch
          if (formulaCells.length > 0) {
            await quadraticCore.setFormulas({ sheetId, formulas: formulaCells });
          }
        } else {
          // No empty or formula cells — use fast batch setCellValues
        ...
        // Deep-Traceability: ALWAYS store provenance for every cell
        {
          const sourceUrls = (provArgs._sourceUrls as Array<{ title: string; uri: string }>) || [];
          const responseText = (provArgs._responseText as string) || '';
          const primarySource = sourceUrls[0];

          console.log('[Deep-Traceability:SetCellValues] provArgs keys:', Object.keys(provArgs));
          console.log('[Deep-Traceability:SetCellValues] _sourceUrls:', sourceUrls);
          console.log('[Deep-Traceability:SetCellValues] provenanceMeta:', provenanceMeta);

          // Priority: 1) Gemini _provenance metadata, 2) grounding URLs, 3) fallback
          const baseProv: Omit<Provenance, 'location'> = {
            fileId: provenanceMeta?.source_url || primarySource?.uri || `ai-cell-values-${Date.now()}`,
            fileName: provenanceMeta?.file_name || primarySource?.title || 'Data Source',
            fileType:
              provenanceMeta?.source_url || primarySource?.uri
                ? 'url'
                : (provenanceMeta?.file_type as SourceFileType) || 'text',
  ...
  [AITool.SetFormulaCellValue]: async (args) => {
    try {
      ...
      for (const formula of formulas) {
        ...
        // Strip leading '=' from formula strings - the core parser expects formulas without it
        const codeString = formula.formula_string.startsWith('=')
          ? formula.formula_string.slice(1)
