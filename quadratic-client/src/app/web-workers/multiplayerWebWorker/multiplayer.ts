...
export class Multiplayer {
  ...
  async init(fileId: string, user: User, anonymous: boolean) {
    ...
    // In local mode there is no multiplayer server, but we still need to
    // initialise the core-multiplayer channel so that offline transactions
    // are loaded and the `offlineTransactionsApplied` / `multiplayerSynced`
    // events fire, which unblocks the QuadraticApp loading gate.
    if (import.meta.env.VITE_AUTH_TYPE === 'local') {
      this.state = 'connected';
      const channel = new MessageChannel();
      quadraticCore.initMultiplayer(channel.port2);
      // Emit multiplayerSynced so QuadraticApp stops waiting
