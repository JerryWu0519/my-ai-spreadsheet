...
export class MultiplayerServer {
  ...
  private async connect(skipFetchJwt = false) {
    // Skip multiplayer in local dev mode — no server to connect to.
    // We intentionally do NOT set state here; `multiplayer.init()` on the
    // main thread checks `this.state` and returns early if it's 'connected',
    // which would prevent the core-multiplayer init chain from running.
    // Instead, the main thread handles local mode directly in multiplayer.init().
    if (import.meta.env.VITE_AUTH_TYPE === 'local') {
      return;
    }
