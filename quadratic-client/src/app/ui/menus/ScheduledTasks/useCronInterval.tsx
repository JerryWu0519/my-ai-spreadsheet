...
import type { CronFieldCollection, DayOfWeekRange } from 'cron-parser';
...
export const UseCronInterval = (initialCron?: string, timezone?: string): CronInterval => {
  ...
  const changeDaysTime = useCallback(
    (time: string) => {
      ...
      // Build a cron string by overriding the hour/minute while preserving the dayOfWeek
      const parsedFields = CronExpressionParser.parse(cron).fields;
      const dayOfWeekPart = parsedFields.dayOfWeek.isWildcard ? '1-7' : parsedFields.dayOfWeek.values.join(',');
      const newCronString = `${convertedMinute} ${convertedHour} * * ${dayOfWeekPart}`;
      setCron(newCronString);
    },
    [cron, fields, tz]
  );
    ...
    (day: string) => {
      ...
      setCron((prevCron) => {
        ...
        const newValues = fields.dayOfWeek.values.includes(value)
          ? [...fields.dayOfWeek.values.filter((v: number) => v !== value)]
          : [...fields.dayOfWeek.values.filter((v: number) => v !== value), value];

        // can't have 0 days
        if (newValues.length === 0) return prevCron;
        // Build cron string preserving minute/hour and overriding dayOfWeek
        const minutePart = fields.minute.isWildcard ? '*' : String(fields.minute.values[0]);
        const hourPart = fields.hour.isWildcard ? '*' : String(fields.hour.values[0]);
        const dayPart = newValues.length >= 7 ? '1-7' : newValues.join(',');
        const newCron = `${minutePart} ${hourPart} * * ${dayPart}`;
        return newCron;
      });
    },
    [setCron, fields]
...
export const getCronToListEntry = (cron: string, timezone?: string): string => {
  ...
  if (!fields.hour.isWildcard && !fields.minute.isWildcard) {
    ...
    if (fields.dayOfWeek.isWildcard || fields.dayOfWeek.values.length >= 7) {
      ...
      fields.dayOfWeek.values.every((day: number) => !isNaN(Number(day)) && Number(day) > 0 && Number(day) < 6)
    ) {
      days = 'Every weekday';
    } else if (
      fields.dayOfWeek.values.length === 2 &&
      fields.dayOfWeek.values.every((day: number) => day === 0 || day === 6)
    ) {
      days = 'Every weekend';
    } else {
      days = joinListWith({
        arr: fields.dayOfWeek.values.flatMap((day: number) => (!isNaN(Number(day)) ? [DAYS_STRING[Number(day)]] : [])),
        conjunction: 'and',
      });
    }
    const { hour, minute } = hourMinuteUTCToTimezone(fields.hour.values[0], fields.minute.values[0], tz);
