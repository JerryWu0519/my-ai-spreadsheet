...
impl GridController {
    ...
    pub fn set_formula_operations(
        ...
        // Strip leading '=' if present - the formula parser expects formulas
        // without the '=' prefix (e.g., "B2/B3" not "=B2/B3")
        let code_string = if let Some(stripped) = code_string.strip_prefix('=') {
            stripped.to_string()
        } else {
            code_string
        };
